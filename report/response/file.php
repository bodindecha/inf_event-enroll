<?php
	...
	if (!has_perm("admission")) $error = 901;
	else if (!isset($_REQUEST["of"]) || !isset($_REQUEST["type"])) $error = 902;
	else{
		...
		$authuser = escapeSQL($_REQUEST["of"]);
		...
		if (!$getExtn) $error = 905;
			...
			if (!file_exists("../../resource/upload/$path")) $error = 404;
		} $db -> close();
	}
