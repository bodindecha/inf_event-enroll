							...
							switch ($group) {
								...
								if (!$getinfo) {
									...
									$readinfo = $getinfo -> fetch_array(MYSQLI_ASSOC); $data = array(
										...
										"impact" => $vToken -> create($readinfo["datid"])."+".(TianTcl::encrypt("sef-".($group == "new" ? $readinfo["signature"] : "4n"), nest: 2))
			...
			switch ($command) {
				case "loadFilterOpt": {
						...
						if ($get -> num_rows) while ($eo = $get -> fetch_assoc()) {
							...
							$options = array(array(), $options); $idx = 1;
							foreach ($CV_groupAdmShort as $type) array_push($options[0], array("ref" => strval($idx++), "title" => $type));
