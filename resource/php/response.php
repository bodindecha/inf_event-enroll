<?php
	...
	if (isset($_REQUEST["filter"]) && isset($_REQUEST["page"]) && isset($_REQUEST["show"]) && isset($_REQUEST["sortBy"]) && isset($_REQUEST["sortOrder"])) {
		...
		if (isset($_REQUEST["list"])) {
			...
			switch ($list) {
				case "new": {
					...
					$queryBegin = "SELECT a.stdid,(CASE a.timerange WHEN 5 THEN 3 ELSE a.timerange END) AS timerange,(CASE a.choose WHEN 'Y' THEN 'ยืนยันสิทธิ์' WHEN 'N' THEN 'สละสิทธิ์' ELSE 'ยังไม่ใช้สิทธิ์' END) AS choose2,a.filetype,c.namep,c.namefth,c.namelth,c.room FROM admission_present a INNER JOIN $mainDBname.user_s c ON a.stdid=c.stdid";
					...
					$queryBegin = "SELECT a.stdid,b.name AS name1,d.name AS name2,a.filetype,c.namep,c.namefth,c.namelth,c.room FROM admission_change a INNER JOIN admission_sgroup b ON a.type=b.code INNER JOIN $mainDBname.user_s c ON a.stdid=c.stdid LEFT JOIN admission_sgroup d ON a.choose=d.code";
		...
		if ($list == "new") $group = array(
			$db -> real_escape_string(trim($_REQUEST["filter"]["group"][0])),
			$db -> real_escape_string(trim($_REQUEST["filter"]["group"][1]))
		); else $group = $db -> real_escape_string(trim($_REQUEST["filter"]["group"]));
		$sql = $queryPreset;
		if ($list == "new") {
			...
			} if ($group[0]<>"*") $sql .= " AND a.type=".$group[0];
			if ($group[1]<>"*") $sql .= " AND a.timerange=".$group[1];
		} else if ($list == "prs") {
			if ($class<>"*") switch ($class) {
				case "ans": $sql .= " AND a.choose IS NOT NULL"; break;
			...
			if ($class<>"*") switch ($class) {
				...
				case "C": $sql .= " AND a.choose='C'"; $col = array("F", "A", "B", "C", "D", "E"); break;
